...
print(f"Coefficient of drag = {steady_state_force / coeff:.6e}")

fig, ax = plt.subplots(figsize=(12, 6))
fig.patch.set_facecolor('#0d1117')
ax.set_facecolor('#0d1117')

ax.plot(time, force / coeff, label="Drag Coefficient", color=plt.cm.tab10.colors[0], linewidth=1.4)
ax.plot(time, np.zeros(len(time)), color='#8b949e', linewidth=1.0, alpha=0.8)
ax.plot(time, np.ones(len(time)) * steady_state_force / coeff,
        label="Steady State Drag Coefficient", color=plt.cm.tab10.colors[1], linewidth=1.4)

ax.set_ylim(2, 9)
ax.set_xlabel('Time [s]', color='#c9d1d9', fontsize=12)
ax.set_ylabel('Drag Coefficient', color='#c9d1d9', fontsize=12)
ax.set_title('Drag Coefficient vs Time', color='#e6edf3', fontsize=13, pad=12)
